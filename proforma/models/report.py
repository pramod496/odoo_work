...
class ReportMyReport(models.AbstractModel):
    _name = 'report.proforma.report_account_invoice'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        lot_id = self.env['stock.picking'].search([('name','=',docs.origin)])
        ori_id = self.env['sale.order'].search([('origin','=',docs.origin)]) 
                ...
                if j.hsn_code in hsn:
                    for l in hsn_list:
                        if l['hsn_code'] == j.hsn_code:
                            if tax_type == "state":
                                for k in j.tax_ids:
                                    l['total_taxable_amount'] += j.price_total
                                    # print('j.price_total',j.price_total)
                                ...
                                l['total_taxable_amount'] += j.price_total
                                # print('j.price_total integ', j.price_total)

                                l['tax_amount_igst'] += j.price_total
                                l['taxable_value'] += j.price_subtotal
                            else :
                    ...
                    if tax_type == "state":
                        ...
                        hsn_list.append({
                            ...
                            'tax_amount_sgst': j.price_total/2,
                            'tax_rate_cgst': tax_rate_cgst,
                            'tax_amount_cgst': j.price_total/2,
                            'total_taxable_amount': j.price_total,
                            })
                    elif tax_type == "integ":
                        hsn_list.append({
                            'hsn_code':j.hsn_code,
