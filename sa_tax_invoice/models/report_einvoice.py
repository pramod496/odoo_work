...
class ReportMyReport(models.AbstractModel):
    _name = 'report.sa_tax_invoice.report_tax_einvoice_template'

    sale_id_rel = fields.Many2one('sale.order')

    def trueround_precision(self, number, places=0, rounding=None):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if origin:
            orig = origin.split(', ')
            for ori in orig:
                # sale = self.env['sale.order'].search([('client_order_ref', '=', docs.ref)])
                # sale = self.env['sale.order'].search([('client_order_ref','=', ori)])
                sale = self.env['sale.order'].search([('name','=', ori)])
                # for i in sale:
                #     self.sale_id_rel = i
                a.append(i.client_order_ref)
                    # a.append(sale.client_order_ref)
                # if i.po_date:
                if sale.po_date == True:
                     b.append(i.po_date.strftime("%d-%m-%Y"))
        ...
        stock_id =[]
        for i in sale_id:
            stock_id = self.env['stock.picking'].search([('origin', '=', i.name)])
        hsn = []
        hsn_list = []
        tax_type = ""
        for i in docs:
                        ...
                        for hsn_val in hsn:
                            ...
                            if len(product_hsn) > 1:
                                ...
                                if len(product_hsn) == 1:
                                    ...
                                    if l['hsn_code'] == hsn_val:
                                        # print("!!!!!!!!!!!!", val, price, single_hsn.invoice_line_tax_ids)
                                        for k in single_hsn.tax_ids:
                                            # print("!!!!!!in loop!!!!!!", k.amount)
                                            price = price + ((val * k.amount) / 100)
                                        # print("###################", price, math.ceil(price), round(533.925, 2))
                                        # print("###################", self.trueround_precision(price, 2))
                                        price = float(self.trueround_precision(price, 2))
                                        if tax_type == "state":
                                            if docs.amount_tax > 0.0:
                                                l['taxable_value'] = val
                                                l['total_taxable_amount'] = price
